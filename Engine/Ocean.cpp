...
bool Ocean::Initialize(ID3D11Device* device, int terrainWidth, int terrainHeight)
{
	int index;
	float height = 0.0;
	bool result;
	...
	for (int j = 0; j < m_terrainHeight; j++)
	{
		for (int i = 0; i < m_terrainWidth; i++)
		{
			...
			m_heightMap[index].y = (float)height-1.5;
...
void Ocean::Render(ID3D11DeviceContext* deviceContext)
{
...
bool Ocean::InitializeBuffers(ID3D11Device* device)
{
	VertexType* vertices;
...
void Ocean::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
...
bool Ocean::GenerateHeightMap(ID3D11Device* device)
{
	bool result;
	...
	for (int j = 0; j < m_terrainHeight; j++)
	{
		for (int i = 0; i < m_terrainWidth; i++)
		{
			...
			m_heightMap[index].y = perlinNoise.Noise(j / scale * m_frequency, i / scale * m_frequency, 1);
